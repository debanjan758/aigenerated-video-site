...
export function generateClips(videoId: string, videoInfo: VideoInfo): Clip[] {
  ...
  for (let i = 0; i < 100; i++) {
    ...
    const copyrightRiskIndex = Math.floor(rand() * copyrightLevels.length);
    const copyrightRisk = copyrightLevels[copyrightRiskIndex];

    const clipDuration = 15 + Math.floor(rand() * 44);
    const maxStart = totalDurationSeconds - clipDuration;
    const startSeconds = Math.floor(rand() * maxStart);
    const endSeconds = startSeconds + clipDuration;

    const viralScore = Math.floor(60 + rand() * 39);
    const hookStrength = Math.floor(65 + rand() * 35);
    const shareability = Math.floor(60 + rand() * 40);
    const retention = Math.floor(55 + rand() * 45);

    const viewRangeIdx = Math.floor(rand() * viewRanges.length);
    const thumbnailIdx = Math.floor(rand() * thumbnailColors.length);
